...
__loadDB2Docker()
{
   ...
   if [[ ! `docker images --quiet store/ibmcorp/db2_developer_c:11.1.4.4-ppc64le` ]]; then
     ...
     docker run -h db2server_  --name $DB2_DOCKER_NAME --restart=always  --detach   --privileged=true -p 50000:50000 -p 55000:55000  -e LICENSE=accept -e DB2INSTANCE=db2inst1 -e DB2INST1_PASSWORD=password store/ibmcorp/db2_developer_c:11.1.4.4-ppc64le db2start
